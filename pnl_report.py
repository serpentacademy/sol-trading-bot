...
def generate_pnl_report():
    ...
    # 5. PnL Calculation
    net_pnl_usdc = final_total_value_usdc - initial_total_value_usdc
    pnl_percentage = (net_pnl_usdc / initial_total_value_usdc) * 100 if initial_total_value_usdc > 0 else 0
    ...
    pnl_table.add_row("Current Value", f"${final_total_value_usdc:.2f}")
    
    console.print(pnl_table)
    
    # Final Result Panel
    pnl_color = "bold green" if net_pnl_usdc >= 0 else "bold red"
    pnl_sign = "+" if net_pnl_usdc >= 0 else ""
    
    result_text = Text()
    result_text.append(f"Total Network Fees Paid: {total_fees_sol:.6f} SOL\n", style="dim orange1")
    result_text.append(f"NET PROFIT / LOSS: ", style="bold white")
    result_text.append(f"{pnl_sign}${net_pnl_usdc:.2f} ({pnl_sign}{pnl_percentage:.2f}%)", style=pnl_color)
    
    console.print(Panel(result_text, title="Bottom Line", border_style=pnl_color))
    console.print("\n")
